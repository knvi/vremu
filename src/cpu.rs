...
impl Cpu {
    ...
    // update physical page number and addressing mode
    fn update_paging(&mut self, csr_addr: usize) {
        if csr_addr != SATP {
            return;
        }

        // read ppn of the root page table; supervisor physical address divided by 4 KiB
        self.page_table = (self.load_csr(SATP) & ((1 << 44) - 1)) * PAGE_SIZE;

        // read MODE field, which selects address-translation mode
        let mode = self.load_csr(SATP) >> 60;

        // enable SV39 paging if value of mode is 8
        if mode == 8 {
