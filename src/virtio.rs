...
// Interrupt request for virtio
pub const VIRTIO_IRQ: u64 = 1;

const VRING_DISC_SIZE: u64 = 16;
// number of virtio descriptors .must be power of 2
...
impl Virtio {
    ...
    // load 4 bytes from virtio only if addr is valid. otherwise return 0
    ...
    // store 4 bytes to virtio only if addr is valid. otherwise, do nothing
    ...
    // access disk via virtio. associated function which takes a cpu object to read and write
    // with a memory directly (DMA)
    pub fn disk_access(cpu: &mut Cpu) {
        ...
        // avail[0] is flags and avail[1] tells how far to look in avail[2..]
        let offset = cpu.bus.load(avail_addr.wrapping_add(1), 16).expect("failed to read offset");
        // avail[2] are desc[] indices the device should process
        // we tell the device the first index in our chain of descriptors
        let index = cpu.bus.load(avail_addr.wrapping_add(offset % DESC_NUM).wrapping_add(2), 16).expect("failed to read index");
        // read vringdesc, virtio descriptors.
        let desc_addr0 = desc_addr + VRING_DISC_SIZE * index;
        let addr0 = cpu.bus.load(desc_addr0, 64).expect("failed to read an address field in a descriptor");
        ...
        let next0 = cpu.bus.load(desc_addr0.wrapping_add(14), 16).expect("failed to read a next field in a descriptor");

        // read vringdesc again
        let desc_addr1 = desc_addr + VRING_DISC_SIZE * next0;
        let addr1 = cpu.bus.load(desc_addr1, 64).expect("failed to read an address field in a descriptor");
        let len1 = cpu.bus.load(desc_addr1.wrapping_add(8), 32).expect("failed to read a len field in a descriptor");
        let flags1 = cpu.bus.load(desc_addr1.wrapping_add(12), 16).expect("failed to read a flags field in a descriptor");
        ...
        //   uint64 sector;
        // } buf0;
        let blk_sector = cpu.bus.load(addr0.wrapping_add(8), 64).expect("failed to read a sector field in a vitio_blk_outhdr");

        // write to device if the second bit of flag1 is set
        match (flags1 & 2) == 0 {
            true => {
                // read mem data and write to disk (DMA)
                for i in 0..len1 as u64 {
                    let data = cpu.bus.load(addr1 + i, 8).expect("failed to read from mem");
                    cpu.bus.virtio.write_disk(blk_sector * 512 + i, data);
                }
            }
            false => {
                // read disk data and write to mem 
                for i in 0..len1 as u64 {
                    let data = cpu.bus.virtio.read_disk(blk_sector * 512 + i);
                    cpu.bus.store(addr1 + i, data, 8).expect("failed to write to mem");
                }
            }
        }  
        ...
        cpu.bus.store(used_addr.wrapping_add(2), 16, new_id % 8).expect("failed to write to mem");
