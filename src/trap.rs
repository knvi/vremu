...
pub trait Trap {
    fn exception_code(&self) -> u64; // return exception code that identifies the exception

    fn take_trap(&self, cpu: &mut Cpu); // take the trap
    ...
    fn take_trap_helper(&self, cpu: &mut Cpu, is_interrupt: bool) {
            ...
            && ((cpu.load_csr(MEDELEG).wrapping_shr(cause as u32)) & 1 != 0)
        {
            ...
            // set program counter to the supervisor trap-handler base address (WHICH IS STVEC)
            if is_interrupt {
            ...
            // Clear a global interrupt-enable bit for supervisor mode (SIE, 1).
            cpu.store_csr(SSTATUS, cpu.load_csr(SSTATUS) & !(1 << 1));

            // 4.1.1 Supervisor Status Register (sstatus)
            // "When a trap is taken into S-mode, sstatus is written with a new value that
            // disables interrupts and sets the previous privilege mode to the current
            // privilege mode.  Otherwise, sstatus is never written by the implementation,
